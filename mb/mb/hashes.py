...
class Hasheable:
    ...
    def __init__(
        ...
        dst_dir=None,
        base_dir=None,
        do_zsync_hashes=False,
        do_chunked_hashes=True,
        chunk_size=DEFAULT_PIECESIZE,
    ):
        ...
        self.dst_basename = f"{self.basename}.size_{self.size}"
    ...
    def check_file(
        self, verbose=False, dry_run=False, force=False, copy_permissions=True
    ):
    ...
    def check_db(self, conn, verbose=False, dry_run=False, force=False):
        ...
        c.execute("SELECT id FROM filearr WHERE path = %s LIMIT 1", [self.src_rel])
        ...
        if res_filearr:
            ...
            c.execute(
                "SELECT file_id, mtime, size FROM hash WHERE file_id = %s LIMIT 1",
                [file_id],
            )
            res_hash = c.fetchone()
        else:
            print(
                "File %r not in database. Not on mirrors yet? Will be inserted."
                % self.src_rel
            )
            file_id = None
            res_hash = None
        ...
        if not res_hash:
            ...
            if not res_filearr:
                c.execute(
                    "INSERT INTO filearr (path, mirrors) VALUES (%s, '{}')",
                    [self.src_rel],
                )
            ...
            c.execute(
                """INSERT INTO hash (file_id, mtime, size, md5,
            ...
            c.execute(
                """UPDATE hash set mtime = %s, size = %s,
                                         md5 = decode(%s, 'hex'),
                                         sha1 = decode(%s, 'hex'),
                                         sha256 = decode(%s, 'hex'),
                                         sha1piecesize = %s,
...
class HashBag:
    ...
    def get_zparams(self):
        if self.zseq_matches and self.zrsum_len and self.zchecksum_len:
            return f"{self.zseq_matches},{self.zrsum_len},{self.zchecksum_len}"
        else:
            return ""
    ...
    def zs_guess_zsync_params(self):
        ...
        rsum_len = math.ceil(
            ((math.log(size or 1) + math.log(blocksize)) / math.log(2) - 8.6)
            / seq_matches
            / 8
        )
