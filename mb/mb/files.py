...
def has_file(conn, path, mirror_id):
    ...
    query = "SELECT path FROM filearr WHERE path {} '{}' AND {} = ANY(mirrors)".format(
        oprtr,
        path,
        mirror_id,
    )
...
def add(conn, path, mirror):
    query = """SELECT mirr_add_bypath(%d, '%s')""" % (mirror.id, path)
...
def dir_show_mirrors(conn, path, missing=False):
    ...
    mirror_ids = []
    for i in result:
        for mirror_id in i[0]:
            mirror_id_str = str(mirror_id)
            if mirror_id_str not in mirror_ids:
                mirror_ids.append(mirror_id_str)

    if not mirror_ids:
        return []
    if not missing:
        query = (
            """select identifier from server where enabled and id in (%s)"""
            % ",".join(mirror_ids)
        )
    else:
        query = (
            """select identifier from server where enabled and id not in (%s)"""
            % ",".join(mirror_ids)
        )
    result = conn.Server._connection.queryAll(query)
...
def dir_filelist(conn, path):
    ...
    query = (
        """SELECT filearr.path, hash.file_id
                   FROM filearr
               LEFT JOIN hash
                   ON hash.file_id = filearr.id
               WHERE filearr.path ~ '^"""
        + util.pgsql_regexp_esc(path)
        + """/[^/]*$'"""
    )
