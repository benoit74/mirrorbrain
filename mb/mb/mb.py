...
class MirrorDoctor(cmdln.Cmdln):
    def get_optparser(self):
        ...
        optparser.add_option(
            ...
            help="location of configuration file (default: /etc/mirrorbrain.conf)",
        )
        optparser.add_option(
            "-d", "--debug", action="store_true", help="print info useful for debugging"
        )
        optparser.add_option(
            "-b",
            "--brain-instance",
    ...
    def postoptparse(self):
        ...
        self.conn = mb.conn.Conn(
            self.config.dbconfig, version=version, debug=self.options.debug
        )
    ...
    @cmdln.option("-C", "--comment", metavar="ARG", help="comment string")
    @cmdln.option(
        "--operator-name",
        metavar="ARG",
    ...
    @cmdln.option("-a", "--admin", metavar="ARG", help="admins's name")
    @cmdln.option("-e", "--admin-email", metavar="ARG", help="admins's email address")
    @cmdln.option(
        "-s",
        "--score",
        default=100,
    ...
    @cmdln.option("-F", "--ftp", metavar="URL", help="FTP base URL")
    @cmdln.option(
        "-R", "--rsync", metavar="URL", help="rsync base URL (starting with rsync://)"
    )
    @cmdln.option("-H", "--http", metavar="URL", help="HTTP base URL")
    @cmdln.option(
        "-r", "--region", metavar="ARG", help="two-letter region code, e.g. EU"
    )
    @cmdln.option(
        "-c", "--country", metavar="ARG", help="two-letter country code, e.g. DE"
    )
    def do_new(self, subcmd, opts, identifier):
        ...
        try:
            ...
            self.conn.Server.select(self.conn.Server.q.identifier == identifier)[0]
        except IndexError:
            pass
        else:
            sys.exit(
        ...
        s = self.conn.Server(
            ...
            otherCountries="",
            publicNotes="",
            comment=opts.comment or "Added - %s" % time.ctime(),
            scanFpm=0,
    ...
    @cmdln.option(
        "--other-countries",
        action="store_true",
        help="also display other countries that a mirror is configured to handle",
    )
    @cmdln.option("--country", action="store_true", help="also display the country")
    @cmdln.option("--region", action="store_true", help="also display the region")
    @cmdln.option(
        "--prefix", action="store_true", help="also display the network prefix"
    )
    @cmdln.option("--asn", action="store_true", help="also display the AS")
    @cmdln.option("--prio", action="store_true", help="also display priorities")
    @cmdln.option(
        "-R", "--rsync-url", action="store_true", help="also display the rsync URL"
    )
    @cmdln.option(
        "-F", "--ftp-url", action="store_true", help="also display the FTP URL"
    )
    @cmdln.option(
        "-H", "--http-url", action="store_true", help="also display the HTTP URL"
    )
    @cmdln.option("--disabled", action="store_true", help="show only disabled mirrors")
    @cmdln.option(
        "-a",
        "--show-disabled",
        action="store_true",
    ...
    @cmdln.option("-c", metavar="XY", help="show only mirrors whose country matches XY")
    @cmdln.option(
        "-r",
        metavar="XY",
    ...
    @cmdln.option(
        "-p", "--prefix", action="store_true", help="print the network prefix"
    )
    @cmdln.option("-a", "--asn", action="store_true", help="print the AS number")
    def do_iplookup(self, subcmd, opts, ip):
        ...
        if opts.asn:
            ...
            print(f"{r.prefix} (AS{r.asn}) {r.ip6}")
    ...
    @cmdln.option(
        "-p", "--prefix", action="store_true", help="update the network prefix"
    )
    @cmdln.option("-a", "--asn", action="store_true", help="update the AS number")
    @cmdln.option(
        "-c",
        "--coordinates",
        action="store_true",
    ...
    def do_update(self, subcmd, opts, *args):
        ...
        if opts.all:
            opts.asn = (
                opts.prefix
            ) = opts.coordinates = opts.country = opts.region = True

        if not (
            opts.asn or opts.prefix or opts.coordinates or opts.country or opts.region
        ):
            sys.exit(
        ...
        if not args:
            if opts.all_mirrors:
                ...
                mirrors = self.conn.Server.select(
                    AND(self.conn.Server.q.statusBaseurl, self.conn.Server.q.enabled)
                )
        ...
        for mirror in mirrors:
            ...
            if opts.prefix and res:
                if not res.prefix:
                    print(
                        "{}: STRANGE! There's no prefix containing this hosts IP address ({})...".format(
                            mirror.identifier, res.ip
                        )
                    )
                elif mirror.prefix != res.prefix:
                    print(
                        "{}: updating network prefix ({} -> {})".format(
            ...
            if opts.asn and res:
                if not res.asn:
                    print(
                        "{}: STRANGE! There's no ASN containing this hosts IP address ({})...".format(
                            mirror.identifier, res.ip
                        )
                    )
                elif mirror.asn != res.asn:
                    print(
                        "{}: updating autonomous system number ({} -> {})".format(
            ...
            if opts.coordinates:
                ...
                if float(mirror.lat or 0) != lat or float(mirror.lng or 0) != lng:
                    print(
                        "{}: updating geographical coordinates ({} {} -> {} {})".format(
    ...
    @cmdln.option(
        "--content", action="store_true", help="download and show the content"
    )
    @cmdln.option("--md5", action="store_true", help="download and show the md5 sum")
    @cmdln.option(
        "--urls",
        dest="url_type",
        metavar="TYPE",
    ...
    @cmdln.option("-m", "--mirror", help="probe only on this mirror")
    @cmdln.option(
        "-a",
        "--all-mirrors",
        action="store_true",
    ...
    def do_probefile(self, subcmd, opts, filename):
        ...
        try:
            mirrors_have_file = mb.testmirror.mirrors_have_file(
                mirrors,
                filename,
...
"""
        ...
        if not new:
            ...
            for i in mb.conn.server_editable_attrs:
                ...
                if (old_dict[i] and not new_dict[i]) or (
                    ...
                    if not new_dict[i]:
                        print(f"unsetting {i} (was: {old_dict[i]!r})")
                    else:
                        print(f"changing {i} from {old_dict[i]!r} to {new_dict[i]!r}")
    ...
    @cmdln.option(
        "-e", "--edit", action="store_true", help="edit the markers in $EDITOR"
    )
    @cmdln.option(
        "-d",
        "--delimiter",
    ...
    def do_markers(self, subcmd, opts):
        ...
        old = [
            "{} {} {}".format(
                i.subtreeName, opts.delimiter, " ".join(i.markers.split())
            )
            for i in markers
        ]
        old = "\n".join(old) + "\n"
        ...
        boilerplate = f"""\
#
# Note: {opts.delimiter!r} delimits subtree name and marker file(s).
# Example:
#
# Factory {opts.delimiter} factory/repo/oss/content
# PPC     {opts.delimiter} ppc/factory/repo/oss/content
# BS      {opts.delimiter} repositories/server:mail.repo repositories/Apache.repo
...
"""
        ...
        if not new:
            ...
            for marker in markers:
                self.conn.Marker.delete(marker.id)
            ...
            for line in new.splitlines():
                ...
                if not lineStripped or lineStripped.startswith("#"):
                    continue
                try:
                    name, markers = lineStripped.split(opts.delimiter, 1)
                except Exception:
                    sys.exit("Parse error")

                self.conn.Marker(
                    subtreeName=name.strip(), markers=" ".join(markers.split())
                )
    ...
    @cmdln.option("-C", "--comment", metavar="ARG", help="comment string to append")
    ...
    @cmdln.option(
        ...
        action="count",
        default=0,
        help="Produce less output. Can be given multiple times.",
    )
    @cmdln.option(
        "-v",
        "--verbose",
    ...
    @cmdln.option("-a", "--all", action="store_true", help="Scan all enabled mirrors.")
    @cmdln.option(
        "-j", "--jobs", metavar="N", help="Run up to N scanner queries in parallel."
    )
    @cmdln.option("-S", "--scanner", metavar="PATH", help="Specify path to scanner.")
    @cmdln.option(
        "-d",
        "--directory",
        metavar="DIR",
    ...
    def do_scan(self, subcmd, opts, *args):
        ...
        if opts.sql_debug:
            cmd.append("-S")
        for _i in range(opts.verbosity):
            cmd.append("-v")
        for _i in range(opts.quietness):
            cmd.append("-q")
        ...
        cmd += [
            "-I %s" % i
            for i in self.config.dbconfig.get("scan_top_include", "").split()
        ]
        cmd += [
            "--exclude %s" % i
            for i in self.config.dbconfig.get("scan_exclude", "").split()
        ]
        cmd += [
            "--exclude-rsync %s" % i
            for i in self.config.dbconfig.get("scan_exclude_rsync", "").split()
        ]

        if not opts.all and not args:
            sys.exit(
                "No mirrors specified for scanning. Either give identifiers, or use -a [-j N]."
            )
        ...
        if not opts.directory or len(mirrors) == 1:
            mirrors_to_scan = list(mirrors)
        else:
            print("Checking for existance of %r directory" % opts.directory)
            mirrors_have_file = mb.testmirror.mirrors_have_file(
                mirrors, opts.directory, url_type="scan"
            )
            print()
            for mirror in mirrors:
                for sample in mirrors_have_file:
                    if mirror.identifier == sample.identifier:
                        if sample.has_file:
                            ...
                            if self.options.debug:
                                print(
                                    "{}: directory {} not found. Skipping.".format(
                                        mirror.identifier, opts.directory
                                    )
                                )
            ...
            if len(mirrors_to_scan):
                ...
                print(
                    textwrap.fill(
                        ", ".join([i.identifier for i in mirrors_to_scan]),
                        initial_indent="    ",
                        subsequent_indent="  ",
                    )
                )
        ...
        if opts.enable and rc == 0:
            ...
            for mirror in mirrors_to_scan:
                ...
                print(f"{tt} {mirror.identifier}: testing status of base URL...")
                t = mb.testmirror.access_http(mirror.identifier, mirror.baseurl)
                if t.http_code == 200:
                    mirror.statusBaseurl = 1
                    mirror.enabled = 1
                    print(f"{tt} {mirror.identifier}: OK. Mirror is online now.")
                else:
                    print(
                        "{} {}: Error: base URL does not work: {}".format(
    ...
    @cmdln.option(
        "--force", action="store_true", help="force refreshing all cached hashes"
    )
    @cmdln.option(
        "-n",
        "--dry-run",
    ...
    @cmdln.option(
        "-t", "--target-dir", metavar="PATH", help="set the target directory (required)"
    )
    @cmdln.option("-v", "--verbose", action="store_true", help="show more information")
    def do_makehashes(self, subcmd, opts, startdir):
        ...
        while len(directories_todo) > 0:
            ...
            dst_dir = os.path.join(
                opts.target_dir, src_dir[len(opts.base_dir) :].lstrip("/")
            )
                ...
                if not opts.dry_run:
                    ...
                    try:
                        ...
                        if e.errno == errno.ENOENT:
                            if opts.verbose:
                                print(
                                    "====== skipping %s, which we were about to lock"
                                    % lockfile
                                )
                            continue
                    ...
                    if not opts.dry_run:
                        try:
                            ...
                            if e.errno == errno.EACCES:
                                sys.stderr.write(
                                    "Recursive removing failed for {!r} ({}). Ignoring.\n".format(
                                        i_path, os.strerror(e.errno)
                                    )
                                )
                            else:
                                sys.exit(
                                    "Recursive removing failed for {!r}: {}\n".format(
                                        i_path, os.strerror(e.errno)
                                    )
                                )
            ...
            for i in sorted(dst_remove_db):
                ...
                if dbid:
                    print(f"Obsolete hash in db: {relpath!r} (id {dbid})")
            ...
            if len(ids_to_delete):
                print(
                    "Deleting %s obsolete hashes from hash table" % len(ids_to_delete)
                )
                if not opts.dry_run:
        ...
        if unlinked_files or unlinked_dirs:
            print(
                "Unlinked %s files, %d directories." % (unlinked_files, unlinked_dirs)
            )
    ...
    def do_score(self, subcmd, opts, *args):
        ...
        if not score:
            ...
            print(f"Changing score for {mirror.identifier}: {mirror.score} -> {score}")
    ...
    @cmdln.option("-u", "--url", action="store_true", help="show the URL on the mirror")
    @cmdln.option("-p", "--probe", action="store_true", help="probe the file")
    @cmdln.option("--md5", action="store_true", help="show md5 hash of probed file")
    @cmdln.option("-m", "--mirror", help="apply operation to this mirror")
    def do_file(self, subcmd, opts, action, path):
        ...
        if action == "ls":
            ...
            if opts.probe:
                samples = mb.testmirror.lookups_probe(
                    rows, get_digest=opts.md5, get_content=False
                )
                print()
            else:
                samples = []
    ...
    @cmdln.option(
        "-d", dest="dirpath", metavar="DIR", help="list mirrors on which DIR was found."
    )
    @cmdln.option(
        "--missing",
        dest="missingdirpath",
    ...
    def do_dirs(self, subcmd, opts, *args):
        ...
        if opts.dirpath:
            ...
            for i in mb.files.dir_show_mirrors(
                self.conn, opts.missingdirpath, missing=True
            ):
                print(i[0])
        else:
    ...
    @cmdln.option(
        "-c", "--caption", metavar="STRING", help="insert this string as table caption"
    )
    @cmdln.option(
        "-t",
        "--title",
    ...
    @cmdln.option(
        ...
        "--output",
        metavar="PATH",
        help="write output to this file (tries to do this securely and atomically)",
    )
    @cmdln.option(
        "-I",
        "--inline-images-from",
    ...
    def do_export(self, subcmd, opts, *args):
        ...
        if opts.format == "django" and not opts.project:
            sys.exit(
                "For Django ORM format, specify a project name (roughly corresponding to a MirrorBrain instance) name with --project"
            )
        if opts.format == "vcs" and not opts.target_dir:
            sys.exit(
                "To export for a version control system, specify a target directory"
            )
        ...
        if opts.format == "django":
            ...
            if not os.path.exists(opts.target_dir):
                os.makedirs(opts.target_dir, 750)
            os.chdir(opts.target_dir)
            if not os.path.isdir(".svn"):
                sys.exit(
                    "%r doesn't seem to be a Subversion working copy" % opts.target_dir
                )
            for i in os.listdir("."):
                if i.startswith("."):
                    continue
        ...
        mirrors = self.conn.Server.select()
        for m in mirrors:
            if m.comment is None:
                # print 'null comment', m
                m.comment = ""
            d = mb.conn.server2dict(m)
            d.update({"project": opts.project})
            ...
            if opts.format == "django":
                ...
                s = (
                    "\n".join(
                        [i for i in s.splitlines() if not i.startswith("statusBaseurl")]
                    )
                    + "\n"
                )
        ...
        if opts.format == "vcs" and opts.commit:
            ...
            for line in lines:
                state, i = line.split()
                if state == "!":
                    os.system(f"{opts.commit} delete {i} > /dev/null")
                elif state == "?":
                    os.system(f"{opts.commit} add {i} > /dev/null")

            os.system(
                '{} commit -m "autocommit by mb" {} > /dev/null'.format(
    ...
    @cmdln.option(
        "-u", "--user", dest="user", metavar="USER", help="user owning the timestamp"
    )
    @cmdln.option(
        "-g",
        "--group",
