    ...
    if S.scheme in ["http", "ftp"]:
        ...
        if S.scheme == "http":
            ...
            if S.http_code == 200:
                ...
                print(
                    "unhandled HTTP response code {!r} for URL {!r}".format(
                        S.http_code, S.probeurl
                    )
                )
        elif S.scheme == "ftp":
            ...
            try:
                ...
                sys.stderr.write(f"\n{S.identifier} timed out ({S.probeurl})\n")
                out = ""
...
def make_probelist(
    mirrors, filename, url_type="http", get_digest=False, get_content=False
):
...
def mirrors_have_file(
    mirrors, filename, url_type="all", get_digest=False, get_content=False
):
