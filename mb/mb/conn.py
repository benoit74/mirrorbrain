...
import sqlobject
from sqlobject import SQLObject, dberrors
...
class Conn:
    def __init__(self, config, version, *, debug=False):
        ...
        self.uri = uri_str % (
            ...
            config["dbpass"],
            config["dbhost"],
            config.get("dbport", dbport),
            config["dbname"],
        )

        sqlobject.sqlhub.processConnection = sqlobject.connectionForURI(self.uri)
        ...
        try:
            ...
            try:
                self.Version.select("""component = 'mirrorbrain'""")[0]
            ...
            query = """CREATE TABLE version (
                       ...
                       INSERT INTO version VALUES (1, 'mirrorbrain', {}, {}, {});
                    """.format(
                version.major,
                version.minor,
                version.patchlevel,
        ...
        if self.Version:
            ...
            if (
                mbversion.major < version.major
                or (
                    mbversion.major == version.major and mbversion.minor < version.minor
                )
                or (
                    mbversion.major == version.major
                ...
                print(
                    "found database version {}.{}.{}, older than us: {} >>>>>>>> upgrading".format(
                        mbversion.major, mbversion.minor, mbversion.patchlevel, version
                    )
                )
                query = "UPDATE version SET major={}, minor={}, patchlevel={} WHERE component='mirrorbrain';".format(
        ...
        try:
            ...
            print(
                ">>> A database table for hashes does not exit. Creating...",
                file=sys.stderr,
            )
            query = """
            CREATE TABLE "hash" (
...
def servertext2dict(s):
    ...
    new_attrs = {}
    for a in server_editable_attrs:
