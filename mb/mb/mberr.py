...
class MirrorNotFoundError(MbBaseError):
    ...
    def __init__(self, identifier):
        ...
        self.msg = (
            "A mirror with identifier %r does not exist in the database\n"
            "(full name required for this operation!)" % self.identifier
        )
...
class SocketError(MbBaseError):
    ...
    def __init__(self, url, msg):
        MbBaseError.__init__(self)
        self.url = url
        self.msg = f"Could not access {url!r}: {msg!r}"
...
class NameOrServiceNotKnown(MbBaseError):
    ...
    def __init__(self, hostname):
        MbBaseError.__init__(self)
        self.msg = (
            "DNS lookup for hostname %r failed: Name or service not known" % hostname
        )
