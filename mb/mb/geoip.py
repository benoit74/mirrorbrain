...
def lookup_country_code(addr):
    out = Popen(
        ["geoiplookup", "-f", database, addr], env=ENV, stdout=PIPE  # noqa: S607
    ).communicate()[0]
...
def lookup_region_code(addr):
    try:
        out = Popen(
            ["geoiplookup_continent", "-f", database, addr],  # noqa: S607
            env=ENV,
            stdout=PIPE,
        ).communicate()[0]
    except OSError as e:
        if e.errno == errno.ENOENT:
            sys.exit(
                "Error: The geoiplookup_continent binary could not be found.\n"
...
def lookup_coordinates(addr):
    try:
        out = Popen(
            ["geoiplookup_city", "-f", database, addr],  # noqa: S607
            env=ENV,
            stdout=PIPE,
        ).communicate()[0]
    except OSError as e:
        if e.errno == errno.ENOENT:
            sys.exit(
                "Error: The geoiplookup_city binary could not be found.\n"
